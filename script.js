...
function showPage(page) {
  ...
  if (page === 'projet') document.getElementById('page-projet').classList.remove('hidden');
...
document.getElementById('map-wrap').addEventListener('wheel', e => {
  ...
  const nz = Math.min(Math.max(zoom * (1 - e.deltaY * 0.00025), 0.15), 10);
  const r  = nz / zoom;
...
document.getElementById('map-wrap').addEventListener('gesturechange', e => {
  e.preventDefault(); changeZoom(zoom * e.scale);
}, { passive: false });
...
function initMarkers() {
  ...
  DEPOSITS.forEach((d, i) => {
    ...
    if (d.photo) {
      ...
      // Classe dédiée pour le multiply sur les éléments couleur
  ...
  Object.keys(TYPOLOGIES).forEach(typo => {
    ...
    DEPOSITS.filter(d => d.typo === typo).forEach(d => {
      ...
      if (d.photo) {
        ...
        // Aplat couleur en index — classe multiply dédiée
        item.classList.add('index-item--color');
        const ph = document.createElement('div');
        ph.className = 'index-item-color';
        ph.style.aspectRatio = '4/2';
        ph.style.backgroundColor = TYPO_COLORS[d.typo] || '#888';
        item.appendChild(ph);
      }
